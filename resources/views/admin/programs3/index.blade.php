    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex align-items-center my-4">
                ...
                <a href="{{ route('admin.program3.create') }}" class="btn btn-primary ml-auto"><i class="fas fa-plus"></i> Добавить</a>
            </div>
                    ...
                    <div class="form-row">
                        <div class="form-group col-4">
                            <select name="degree" class="custom-select">
                                ...
                                @include('admin.programs3.partials.select', ['data' => $degrees, 'data_id' => 0])
                            </select>
                        </div>
                        <div class="form-group col-4">
                            <select name="direction" class="custom-select">
                                <option selected="" value="">Направление</option>
                                @include('admin.programs3.partials.select', ['data' => $directions, 'data_id' => 0])
                            </select>
                        </div>
                        <div class="form-group col-4">
                            <select name="faculty" class="custom-select">
                                <option selected="" value="">Факультет</option>
                                @include('admin.programs3.partials.select', ['data' => $faculties, 'data_id' => 0])
                            </select>
                        </div>
                    </div>
                    <div class="form-row">
            ...
            <form class="update_form" action="{{ route('admin.program3_mass_update') }}" method="POST">
                ...
                <table class="border table table-borderless table-light">
                    ...
                    <tbody class="programs_container">
                        ...
                        <tr class="
                            program_item 
                            degree-{{$program->degree->id}} 
                            direction-{{$program->direction->id}} 
                            faculty-{{$program->faculty->id}}
                            @foreach($program->forms as $form) form-{{$form}} @endforeach"
                            data-id="{{$program->id}}">
                            ...
                            <td>
                                @foreach($forms as $form)
                                    @if(in_array($form['id'], $program->forms)) 
                                    <span class="text-success">{{$form['name']}};</span>
                                    @endif
                                @endforeach
                            </td>
                            <td class="text-right">
                                <a href="{{route('admin.program3.show_html', $program->id)}}" class="badge badge-info" data-toggle="tooltip" data-placement="top" title="HTML код">
                                    <i class="fas fa-code"></i>
                                </a>
                                <a href="{{route('admin.program3.edit', $program->id)}}" class="badge badge-primary" data-toggle="tooltip" data-placement="top" title="Редактировать">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <a href="{{route('admin.program3.show', $program->id)}}" class="badge badge-info" data-toggle="tooltip" data-placement="top" title="Подробно">
